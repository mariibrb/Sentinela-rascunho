...
if cod_cliente:
    ...
    with c_e:
        ...
        xe = st.file_uploader("XMLs", type='xml', accept_multiple_files=True, key="xe_v57")
        ge = st.file_uploader("Gerencial", type=['csv'], key="ge_v57")
        ae = st.file_uploader("Autenticidade", type=['xlsx', 'csv'], key="ae_v57")
    with c_s:
        st.subheader("📤 SAÍDAS")
        xs = st.file_uploader("XMLs", type='xml', accept_multiple_files=True, key="xs_v57")
        gs = st.file_uploader("Gerencial", type=['csv'], key="gs_v57")
        as_f = st.file_uploader("Autenticidade", type=['xlsx', 'csv'], key="as_v57")
