...
with st.sidebar:
    ...
    def criar_gabarito():
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            # ICMS
            pd.DataFrame(columns=["NCM", "CST", "ALIQUOTA"]).to_excel(writer, sheet_name='ICMS', index=False)
            for c, v in enumerate(["NCM", "CST", "ALIQUOTA"]): writer.sheets['ICMS'].write(0, c, v, f_header)
            # PIS_COFINS
            pd.DataFrame(columns=["NCM", "DESCRIÇÃO", "CST"]).to_excel(writer, sheet_name='PIS_COFINS', index=False)
            for c, v in enumerate(["NCM", "DESCRIÇÃO", "CST"]): writer.sheets['PIS_COFINS'].write(0, c, v, f_header)
            # IPI
...
if cod_cliente:
    ...
    with c_e:
        ...
        xe = st.file_uploader("XMLs Entrada", type='xml', accept_multiple_files=True, key="xe_v101")
        ge = st.file_uploader("Gerencial Entrada", type=['csv'], key="ge_v101")
        ae = st.file_uploader("Autenticidade Entrada", type=['xlsx', 'csv'], key="ae_v101")
    with c_s:
        st.subheader("📤 SAÍDAS")
        xs = st.file_uploader("XMLs Saída", type='xml', accept_multiple_files=True, key="xs_v101")
        gs = st.file_uploader("Gerencial Saída", type=['csv'], key="gs_v101")
        as_f = st.file_uploader("Autenticidade Saída", type=['xlsx', 'csv'], key="as_v101")
